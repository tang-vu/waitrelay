...
| Long-wait adaptation | `flight-scenery.test.ts` and desktop/mobile `long-wait.spec.ts` | Checks ten-second cycles and continuous crossfades. Browser time advances beyond 65 seconds while the actual server independently completes its 30-second run; Quiet Mode, resume, accepted-choice receipt, and immediate takeover remain functional. |
