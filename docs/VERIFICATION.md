...
| Long-wait adaptation | `flight-scenery.test.ts` and desktop/mobile `long-wait.spec.ts` | Checks ten-second cycles and continuous crossfades. The decorative frame clock advances beyond 65 seconds while host timers and the actual server independently complete the 30-second run; Quiet Mode, resume, accepted-choice receipt, and immediate takeover remain functional. |
